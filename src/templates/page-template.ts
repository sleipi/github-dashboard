...
(function() {
  var ri = document.getElementById('refresh-info');
  if (!ri) return;
  ri.textContent = new Date().toLocaleTimeString('en-GB');
  var _refreshStart = 0;
  ...
  document.addEventListener('htmx:afterSettle', function(e) {
    if (e.detail && e.detail.target && e.detail.target.id === 'cards' && _refreshStart > 0) {
      var took = ((performance.now() - _refreshStart) / 1000).toFixed(1);
      ri.textContent = new Date().toLocaleTimeString('en-GB') + ' (took ' + took + 's)';
      _refreshStart = 0;
    }
  });
})();
