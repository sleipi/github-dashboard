...
describe('GlobalSearchRepo', () => {
  ...
  test('isEnabled defaults to false when no setting stored', () => {
    const { dir, dbPath } = createTempDbPath('gh-dash-global-search-')
    cleanup.push(dir)
    const repos = createSqliteRepos(dbPath)

    expect(repos.globalSearch.isEnabled()).toBe(false)
    repos.close()
  })
  ...
  test('value persists across repo re-open on same db path', () => {
    ...
    repos1.globalSearch.setEnabled(true)
    repos1.close()

    const repos2 = createSqliteRepos(dbPath)
    expect(repos2.globalSearch.isEnabled()).toBe(true)
    repos2.close()
  })
})
